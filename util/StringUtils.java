...
public class StringUtils extends org.apache.commons.lang.StringUtils {
    ...
    /**
     * 解析字符串表达式 解析 出来以后是一个object的数组
     *
     * @param description
     * @param args
     * @return
     */
    public static Object[] analysisDescriptionForObjects(String description, Object[] args) {
        ...
        for (String param1 : result) {
            ...
            pm = pm.replace(">", "");
            String[] params = pm.split(":");
            if (params.length == 1) {//如果没有:分割符
                Integer index = Integer.valueOf(Integer.parseInt(params[0]));
                Object obj = args[index.intValue()];
                resultList.add(obj);//增加元素
            } else {
                Integer index = Integer.valueOf(Integer.parseInt(params[0]));
                String field = params[1];//属性名
                Object o = args[index.intValue()];
                if (o == null) {
                    resultList.add(null);//增加空元素
                } else {
                    Object fieldValue = ReflectionUtils.getFieldValue(o, field);
                    resultList.add(fieldValue);//增加进元素
                }
            }
        }
   ...
   /* public static void main(String[] args){
        ...
        Object[] objects = StringUtils.analysisDescriptionForObjects(param, ags);
